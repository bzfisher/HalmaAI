...
public class PreviousMinimaxPlayer extends Player
{
	...
	public PreviousMinimaxPlayer() { super("random"); }
	public PreviousMinimaxPlayer(String s) { super(s); }
	...
	private CCMove miniMax(CCBoard board, int iterationsLeft)
	{
		...
		double bestScore = PreviousHalmaHeuristics.boardUtility(board, board.getTurn());
	...
	private double minimaxHelper(CCBoard board, int turnPlayer, int actualPlayer, int iterationsLeft)
	{
		...
		if (iterationsLeft<1 || (currentTime-startTime) < 100) return PreviousHalmaHeuristics.boardUtility(board, actualPlayer);

		//otherwise, analyize the possible moves with recursion.
		ArrayList<CCMove> allowedMoves = board.getLegalMoves();
		double topScore = PreviousHalmaHeuristics.boardUtility(board, actualPlayer);
	...
	private double alphaBetaHelper(CCBoard board,  int depth, double alpha, double beta,int turnPlayer, int actualPlayer)
	{
		int currentTime = Calendar.getInstance().get(Calendar.MILLISECOND);
		//if we are at the last turn, return the actual board score.
		if (depth<1 || (currentTime-startTime) < 100) return PreviousHalmaHeuristics.boardUtility(board, actualPlayer);
